...
// https://github.com/mcs-project/mcs/issues/14485 for more info.
...
/// A class that represents a `Get` request.
    ...
    public:
      ...
      void Set(const ObjectID &object_id, std::shared_ptr<McsObject> buffer);
      /// Get the object content for the specific object id.
      std::shared_ptr<McsObject> Get(const ObjectID &object_id) const;
    ...
    private:
      ...
      absl::flat_hash_map<ObjectID, std::shared_ptr<McsObject>> objects_;
    ...
    GetRequest::GetRequest(absl::flat_hash_set<ObjectID> object_ids,
        ...
        : object_ids_(std::move(object_ids)),
          ...
          is_ready_(false) {
      MCS_CHECK(num_objects_ <= object_ids_.size());
    }
    ...
    bool GetRequest::Wait(int64_t timeout_ms) {
      MCS_CHECK(timeout_ms >= 0 || timeout_ms == -1);
      if (timeout_ms == -1) {
...
//      }
    ...
    void GetRequest::Set(const ObjectID &object_id, std::shared_ptr<McsObject> object) {
      std::unique_lock<std::mutex> lock(mutex_);
      if (is_ready_) {
    ...
    std::shared_ptr<McsObject> GetRequest::Get(const ObjectID &object_id) const {
      std::unique_lock<std::mutex> lock(mutex_);
...
//        std::shared_ptr<mcslet::McsletClient> mcslet_client,
        std::function<Status()> check_signals,
        std::function<void(const McsObject &)> unhandled_exception_handler,
        std::function<std::shared_ptr<mcs::McsObject>(
            const mcs::McsObject &object, const ObjectID &object_id)> object_allocator)
        :
//        ref_counter_(std::move(counter)),
//          mcslet_client_(mcslet_client),
    ...
    void CoreWorkerMemoryStore::GetAsync(
        const ObjectID &object_id, std::function<void(std::shared_ptr<McsObject>)> callback) {
      std::shared_ptr<McsObject> ptr;
      {
    ...
    std::shared_ptr<McsObject> CoreWorkerMemoryStore::GetIfExists(const ObjectID &object_id) {
      std::shared_ptr<McsObject> ptr;
      {
        absl::MutexLock lock(&mu_);
        auto iter = objects_.find(object_id);
    ...
    bool CoreWorkerMemoryStore::Put(const McsObject &object, const ObjectID &object_id) {
      std::vector<std::function<void(std::shared_ptr<McsObject>)>> async_callbacks;
      MCS_LOG(DEBUG) << "Putting object into memory store. objectid is " << object_id;
      std::shared_ptr<McsObject> object_entry = nullptr;
      if (object_allocator_ != nullptr) {
        object_entry = object_allocator_(object, object_id);
      } else {
        object_entry = std::make_shared<McsObject>(
...
//        }
        ...
        if (should_add_entry) {
          // If there is no existing get request, then add the `McsObject` to map.
    ...
    Status CoreWorkerMemoryStore::Get(const std::vector<ObjectID> &object_ids,
                                      ...
                                      bool remove_after_get,
                                      std::vector<std::shared_ptr<McsObject>> *results) {
...
//      bool should_notify_mcslet =
//          (mcslet_client_ != nullptr && ctx.ShouldReleaseResourcesOnBlockingCalls());
//
//      // Wait for remaining objects (or timeout).
//      if (should_notify_mcslet) {
//        MCS_CHECK_OK(mcslet_client_->NotifyDirectCallTaskBlocked(/*release_resources=*/true));
//      }
      ...
      int64_t iteration_timeout =
          std::min(timeout_ms, McsConfig::instance().get_timeout_milliseconds());
      if (timeout_ms == -1) {
        iteration_timeout = McsConfig::instance().get_timeout_milliseconds();
      }
...
//      if (should_notify_mcslet) {
//        MCS_CHECK_OK(mcslet_client_->NotifyDirectCallTaskUnblocked());
//      }
    ...
    Status CoreWorkerMemoryStore::Get(
        ...
        int64_t timeout_ms,
        const WorkerContext &ctx,
        absl::flat_hash_map<ObjectID, std::shared_ptr<McsObject>> *results,
        bool *got_exception) {
      const std::vector<ObjectID> id_vector(object_ids.begin(), object_ids.end());
      std::vector<std::shared_ptr<McsObject>> result_objects;
      MCS_RETURN_NOT_OK(Get(id_vector,
                            id_vector.size(),
                            timeout_ms,
                            ctx,
    ...
    Status CoreWorkerMemoryStore::Wait(const absl::flat_hash_set<ObjectID> &object_ids,
      ...
      std::vector<std::shared_ptr<McsObject>> result_objects;
      MCS_CHECK(object_ids.size() == id_vector.size());
    ...
    inline bool IsUnhandledError(const std::shared_ptr<McsObject> &obj) {
    ...
    void CoreWorkerMemoryStore::OnDelete(std::shared_ptr<McsObject> obj) {
    ...
    inline void CoreWorkerMemoryStore::EraseObjectAndUpdateStats(const ObjectID &object_id) {
      ...
      MCS_CHECK(num_in_plasma_ >= 0 && num_local_objects_ >= 0 &&
                num_local_objects_bytes_ >= 0);
      objects_.erase(it);
    }

    inline void CoreWorkerMemoryStore::EmplaceObjectAndUpdateStats(
        const ObjectID &object_id, std::shared_ptr<McsObject> &object_entry) {
    ...
    void CoreWorkerMemoryStore::RecordMetrics() {
      absl::MutexLock lock(&mu_);
//      mcs::stats::STATS_object_store_memory.Record(
//          num_local_objects_bytes_,
//          {{mcs::stats::LocationKey, mcs::stats::kObjectLocWorkerHeap}});
    }
