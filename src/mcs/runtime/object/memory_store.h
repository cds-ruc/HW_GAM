...
//#include "mcs/core_worker/reference_count.h"
...
/// actor call (see direct_actor_transport.cc).
    ...
    public:
      ...
      /// \param[in] mcslet_client If not null, used to notify tasks blocked / unblocked.
      CoreWorkerMemoryStore(
//          std::shared_ptr<ReferenceCounter> counter = nullptr,
//          std::shared_ptr<mcslet::McsletClient> mcslet_client = nullptr,
          std::function<Status()> check_signals = nullptr,
          std::function<void(const McsObject &)> unhandled_exception_handler = nullptr,
          std::function<std::shared_ptr<McsObject>(const McsObject &object,
      ...
      ///
      /// \param[in] object The mcs object.
      ...
      bool Put(const McsObject &object, const ObjectID &object_id);
      ...
      std::shared_ptr<McsObject> GetIfExists(const ObjectID &object_id);
      ...
      void GetAsync(const ObjectID &object_id,
                    std::function<void(std::shared_ptr<McsObject>)> callback);
...
//      FRIEND_TEST(TestMemoryStore, TestMemoryStoreStats);
      ...
      void OnDelete(std::shared_ptr<McsObject> obj);
      ...
      void EmplaceObjectAndUpdateStats(const ObjectID &object_id,
                                       std::shared_ptr<McsObject> &object_entry)
...
//      std::shared_ptr<mcslet::McsletClient> mcslet_client_ = nullptr;

      /// Protects the data structures below.
      mutable absl::Mutex mu_;

      /// Map from object ID to `McsObject`.
      /// NOTE: This map should be modified by EmplaceObjectAndUpdateStats and
      /// EraseObjectAndUpdateStats.
      absl::flat_hash_map<ObjectID, std::shared_ptr<McsObject>> objects_ GUARDED_BY(mu_);
      ...
      absl::flat_hash_map<ObjectID,
          std::vector<std::function<void(std::shared_ptr<McsObject>)>>>
      ...
      std::function<void(const McsObject &)> unhandled_exception_handler_;
