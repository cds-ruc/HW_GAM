...
//            MCS_CHECK(resource_pair.second > 0);
//        }
//    }
...
//            resource_capacity_[resource_pair.first] = FixedPoint(resource_pair.second);
    ...
    ResourceSet::ResourceSet(const std::vector<std::string> &resource_labels,
        ...
        MCS_CHECK(resource_labels.size() == resource_capacity.size());
        for (size_t i = 0; i < resource_labels.size(); i++) {
            MCS_CHECK(resource_capacity[i] > 0);
...
//            const std::string &resource_label = resource_pair.first;
//            const FixedPoint &resource_capacity = resource_pair.second;
//            MCS_CHECK(resource_capacity_.count(resource_label) == 1)
...
//            MCS_CHECK(resource_capacity_[resource_label] >= 0)
...
//                MCS_LOG(DEBUG) << "[AddResourcesCapacityConstrained] Resource "
