...
namespace mcs {
    inline bool operator==(const mcs::rpc::SchedulingStrategy &lhs,
                           const mcs::rpc::SchedulingStrategy &rhs) {
        if (lhs.scheduling_strategy_case() != rhs.scheduling_strategy_case()) {
            return false;
        }

        switch (lhs.scheduling_strategy_case()) {
            case mcs::rpc::SchedulingStrategy::kNodeAffinitySchedulingStrategy: {
            ...
            case mcs::rpc::SchedulingStrategy::kPlacementGroupSchedulingStrategy: {
...
namespace std {
    template <>
    struct hash<mcs::rpc::SchedulingStrategy> {
        size_t operator()(const mcs::rpc::SchedulingStrategy &scheduling_strategy) const {
            size_t hash = std::hash<size_t>()(scheduling_strategy.scheduling_strategy_case());
            if (scheduling_strategy.scheduling_strategy_case() ==
                mcs::rpc::SchedulingStrategy::kNodeAffinitySchedulingStrategy) {
                ...
                hash ^= static_cast<size_t>(
                       ...
                       mcs::rpc::SchedulingStrategy::kPlacementGroupSchedulingStrategy) {
                hash ^= std::hash<std::string>()(
    ...
    template <>
    struct hash<mcs::SchedulingClassDescriptor> {
        size_t operator()(const mcs::SchedulingClassDescriptor &sched_cls) const {
            size_t hash = std::hash<mcs::ResourceSet>()(sched_cls.resource_set);
            hash ^= sched_cls.function_descriptor->Hash();
            hash ^= sched_cls.depth;
            hash ^= std::hash<mcs::rpc::SchedulingStrategy>()(sched_cls.scheduling_strategy);
            return hash;
        }
    };
}  // namespace std
...
/// a executing thread pool.
    struct ConcurrencyGroup {
        ...
        std::vector<mcs::FunctionDescriptor> function_descriptors;

        ConcurrencyGroup() = default;

        ConcurrencyGroup(const std::string &name,
                         uint32_t max_concurrency,
                         const std::vector<mcs::FunctionDescriptor> &fds)
        ...
        std::vector<mcs::FunctionDescriptor> GetFunctionDescriptors() const {
...
/// instead `const TaskSpecification`, since this class is actually mutable.
    ...
    public:
        ...
        mcs::FunctionDescriptor FunctionDescriptor() const;
