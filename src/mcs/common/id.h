...
#include "mcs/common/constants.h"
...
/// https://github.com/mcs-project/mcs/blob/master/src/ray/design_docs/id_specification.md
  ...
  protected:
    BaseID(const std::string &binary) {
      MCS_CHECK(binary.size() == Size() || binary.size() == 0)
...
#define DEFINE_UNIQUE_ID(type)                                                           \
  class MCS_EXPORT type : public UniqueID {                                              \
   public:                                                                               \
...
#pragma pack(pop)
  ...
  T BaseID<T>::FromBinary(const std::string &binary) {
    MCS_CHECK(binary.size() == T::Size() || binary.size() == 0)
  ...
  T BaseID<T>::FromHex(const std::string &hex_str) {
    ...
    if (2 * T::Size() != hex_str.size()) {
      MCS_LOG(ERROR) << "incorrect hex string length: 2 * " << T::Size()
    ...
    for (size_t i = 0; i < T::Size(); i++) {
      ...
      if (err) {
        MCS_LOG(ERROR) << "incorrect hex character, hex string: " << hex_str;
        return T::Nil();
      }
    }
...
#define DEFINE_UNIQUE_ID(type)                                           \
  template <>                                                            \
  struct hash<::mcs::type> {                                             \
    size_t operator()(const ::mcs::type &id) const { return id.Hash(); } \
  };                                                                     \
  template <>                                                            \
  struct hash<const ::mcs::type> {                                       \
    size_t operator()(const ::mcs::type &id) const { return id.Hash(); } \
  };
