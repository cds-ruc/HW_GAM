...
/// Binary representation of a mcs object, consisting of buffer pointers to data and
/// metadata. A mcs object may have both data and metadata, or only one of them.
  class McsObject {
  public:
    /// Create a mcs object instance.
    ///
    /// Set `copy_data` to `false` is fine for most cases - for example when putting
    /// an object into store with a temporary McsObject, and we don't want to do an extra
    /// copy. But in some cases we do want to always hold a valid data - for example, memory
    /// store uses McsObject to represent objects, in this case we actually want the object
    /// data to remain valid after user puts it into store.
    ///
    /// \param[in] data Data of the mcs object.
    /// \param[in] metadata Metadata of the mcs object.
    ...
    McsObject(const std::shared_ptr<Buffer> &data,
    ...
    /// This constructor creates a mcs object instance whose data will be generated
    /// by the data factory.
    McsObject(const std::shared_ptr<Buffer> &metadata,
              const std::vector<rpc::ObjectReference> &nested_refs,
              std::function<std::shared_ptr<mcs::Buffer>()> data_factory,
              bool copy_data = false)
    ...
    /// Create an Mcs error object. It uses msgpack for the serialization format now.
    /// Mcs error objects consist of metadata that indicates the error code (see
    /// rpc::ErrorType) and the serialized message pack that contains serialized
    /// rpc::McsErrorInfo as a body.
    ///
...
//    McsObject(rpc::ErrorType error_type, const rpc::McsErrorInfo *mcs_error_info = nullptr);
    ...
    /// Return the metadata of the mcs object.
...
//    bool IsException(rpc::ErrorType *error_type = nullptr) const;
  ...
  private:
    void Init(const std::shared_ptr<Buffer> &data,
      ...
      MCS_CHECK(data_ || metadata_) << "Data and metadata cannot both be empty.";
    }
