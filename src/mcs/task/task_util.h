...
class TaskSpecBuilder {
  public:
    ...
    TaskSpecBuilder &SetCommonTaskSpec(
      ...
      std::vector<rpc::TaskArg> args) {
      message_->function_descriptor = function_descriptor;
      message_->num_returns = num_returns;
      message_->args.assign(&args[0], 1);
      return *this;
    }

    // TaskSpecBuilder &AddArgs(std::vector<mcs::internal::TaskArg> args) {
    //   std::vector<rpc::TaskArg> rpc_args; 
