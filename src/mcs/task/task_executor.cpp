...
/// Execute remote functions by networking stream.
    msgpack::sbuffer TaskExecutionHandler(const std::string &func_name,
      ...
      if (func_name.empty()) {
        MCS_LOG(ERROR) << "Task function name is empty";
      ...
      do {
        if (actor_ptr) {
          ...
          if (func_ptr == nullptr) {
            MCS_LOG(ERROR) << "unknown function:" << func_name;
...
/// task id etc.
     std::shared_ptr<msgpack::sbuffer> GetExecuteResult(
      ...
      try {
        ...
        MCS_LOG(INFO) << "Get executable function " << func_name << " ok.";
        auto result = entry_function(func_name, args_buffer, actor_ptr);
        MCS_LOG(INFO) << "Execute function " << func_name << " ok.";
        return std::make_shared<msgpack::sbuffer>(std::move(result));
        ...
        MCS_LOG(INFO) << "Mcs intentional system exit while executing function(" << func_name
                       << ").";
        return nullptr;
  ...
  //  }
    ...
    void TaskExecutor::Invoke(
      ...
      for (size_t i = 0; i < task_spec.NumArgs(); i++) {
          msgpack::sbuffer sbuf;
          printf("Invoke data %d , size %d\n", *task_spec.ArgData(i), task_spec.ArgDataSize(i));
          sbuf.write((const char *)task_spec.ArgData(i), task_spec.ArgDataSize(i));
          args_buffer.push_back(std::move(sbuf));
      }
      auto func_name = task_spec.FuncName();
      std::shared_ptr<msgpack::sbuffer> data;
      try {
          auto result =
                  TaskExecutionHandler(func_name, args_buffer, nullptr);
