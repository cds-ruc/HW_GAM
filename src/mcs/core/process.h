...
namespace std {
  ...
  template <>
  struct equal_to<mcs::Process> {
    bool operator()(const mcs::Process &x, const mcs::Process &y) const;
  };

  template <>
  struct hash<mcs::Process> {
    size_t operator()(const mcs::Process &value) const;
  };
}