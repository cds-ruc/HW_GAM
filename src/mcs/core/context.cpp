...
/// per-thread context for core worker.
        struct WorkerThreadContext {
            ...
            ObjectIDIndexType GetNextPutIndex() {
                ...
                // See https://github.com/mcs-project/mcs/issues/10324 for further details.
            ...
            void SetCurrentTask(const TaskSpecification &task_spec) {
                MCS_CHECK(task_index_ == 0);
                MCS_CHECK(put_counter_ == 0);
        ...
        private:
            ...
            /// the current task (via mcs.put or .remote calls). As tasks can be executed
            /// multiple times, this ID represents the n-th execution of the current
            /// task.
            /// See https://github.com/mcs-project/mcs/issues/20713.
        ...
        void WorkerContext::SetCurrentActorId(const ActorID &actor_id) LOCKS_EXCLUDED(mutex_) {
                ...
                if (!current_actor_id_.IsNil()) {
                    MCS_CHECK(current_actor_id_ == actor_id);
                    return;
                }
        ...
        void WorkerContext::SetCurrentTask(const TaskSpecification &task_spec) {
            ...
            MCS_CHECK(current_job_id_ == task_spec.JobId());
            if (task_spec.IsNormalTask()) {
                current_task_is_direct_call_ = true;
            } else if (task_spec.IsActorCreationTask()) {
                if (!current_actor_id_.IsNil()) {
                    MCS_CHECK(current_actor_id_ == task_spec.ActorCreationId());
                }
                ...
                MCS_CHECK(current_actor_id_ == task_spec.ActorId());
            } else {
                MCS_CHECK(false);
            }
            if (task_spec.IsNormalTask() || task_spec.IsActorCreationTask()) {
                ...
                // only set runtime_env_ once and then MCS_CHECK that we
                // never see a new one.
